...
export class tblWMDistribution {
    ...
    @ManyToOne(type=>tblWMRecipient, tblWMRecipient=>tblWMRecipient.tblWmDistributions,{ primary:true, nullable:false,onDelete: 'CASCADE', })
    @JoinColumn({ name:'intWMDistributionWMRecipientID'})
    intWmDistributionWmRecipient:tblWMRecipient | null;


   
    @ManyToOne(type=>tblWMReport, tblWMReport=>tblWMReport.tblWmDistributions,{ primary:true, nullable:false,onDelete: 'CASCADE', })
